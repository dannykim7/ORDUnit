				...
				<input type="hidden" name="perPageNum" value="${pageObject.perPageNum }">
					<div class="input-group">
						<select name="key" class="form-control">
							...
							<option value="w" ${(pageObject.key =="f")?"selected":"" }>파일명</option>
							<option value="tc" ${(pageObject.key =="tc")?"selected":"" }>제목/내용</option>
							<option value="tw" ${(pageObject.key =="tf")?"selected":"" }>제목/파일명</option>
							<option value="cw" ${(pageObject.key =="cf")?"selected":"" }>내용/파일명</option>
						</select>
					</div>
					<div class="input-group">
			...
			<c:forEach items="${list }" var="vo" varStatus="vs">
				<div class="col-md-3">
					<div class="thumbnail dataRow"
						...
						<div class="caption">
							<p>[${vo.no }] ${vo.title }</p>
							${vo.name }(${vo.id }) - <fmt:formatDate value="${vo.writeDate }" pattern="yyyy-MM-dd"/>
						</div>
					</div>
				</div>
